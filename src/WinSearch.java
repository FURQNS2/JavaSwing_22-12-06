...
public class WinSearch extends JDialog {
	...
	public WinSearch() {
		...
		{	
			...
			{
				String columnNames[] = {"번호","주민번호","이름","전화번호","주소","졸업년도","직업"};
	...
	protected void showResult() {  // 검색 결과를 테이블에 출력
		...
		try {
			...
			if( sName.length()!= 0 && sMobile.length() == 0) {
				sql = "SELECT * FROM addresstbl WHERE name LIKE'"+tfName.getText()+"%'";			
			} else if(sName.length() == 0 && sMobile.length() != 0) {
				sql = "SELECT * FROM addresstbl WHERE mobile LIKE'%"+tfmobile.getText()+"%'";							
			} else if(sName.length() == 0 && sMobile.length() == 0){
				sql = "SELECT * FROM addresstbl";
			} else if(sName.length() != 0 && sMobile.length() != 0){
				sql = "SELECT * FROM addresstbl WHERE name LIKE'"+tfName.getText()+"%'AND mobile LIKE'"+tfmobile.getText()+"%'";
			} else {
...
//				record[2] = rs.getString("jumin");
//				record[3] = rs.getString("mobile");
//				record[4] = rs.getString("address");
//				record[5] = rs.getString("gyear");
//				record[6] = rs.getString("company");				
//				dtm.addRow(record);
