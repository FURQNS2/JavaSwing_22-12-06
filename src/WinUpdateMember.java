...
public class WinUpdateMember extends JDialog {
	 ...
	 * Create the dialog.
	 */
	public WinUpdateMember(String sJumin) {
		Jumin = sJumin;
		setTitle("동문회원 가입");
		...
		JButton btnInsert = new JButton("회원 변경");
	...
	private void init(String jumin) { // 받아온 주민번호로 다른 값들 불러오기
		...
		try {
			...
			if(rs.next()) {				
				...
				cbGYear.setSelectedItem(rs.getInt("gyear"));
				tfJumin1.setText(jumin.substring(0,6)); // 주민번호 앞자리
				tfJumin2.setText(jumin.substring(6)); // 주민번호 뒷자리
	...
	protected void updateMember() {
		...
		try {
			...
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/sqlDB", "root", "12345");
			Statement stmt = con.createStatement();	
			String sql = "UPDATE addressTBL SET  name='"+tfName.getText()+"', mobile='"+tfMobile.getText()+"',address='"+tfAddress.getText()+"', company='"+ tfCompany.getText() +"', pic='"+ picPath.replace("\\", "/")+"'";
			
			sql = sql + "WHERE jumin='"+tfJumin1.getText()+String.valueOf(tfJumin2.getPassword()) +"'";
